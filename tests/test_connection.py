...
async def test_connect(mug_update_initial, mock_establish_connection, mug_connection):
    # Already connected
    mug_connection.client = AsyncMock()
    mug_connection.client.is_connected = True
...
async def test_disconnect(mug_connection):
    ...
    mug_connection.client.is_connected = False
    await mug_connection.disconnect()
    mug_connection.client.disconnect.assert_not_called()
...
async def test_get_mug_dsk(mug_connection):
    ...
    mug_connection.client.read_gatt_char.return_value = b'something else'
    assert (await mug_connection.get_dsk()) == "b'something else'"
    mug_connection.client.read_gatt_char.assert_called_with(UUID_DSK)


@pytest.mark.asyncio
async def test_get_mug_temperature_unit(mug_connection):
    mug_connection.client.read_gatt_char.return_value = b'\x01'
    assert (await mug_connection.get_temperature_unit()) == TEMP_FAHRENHEIT
    mug_connection.client.read_gatt_char.assert_called_once_with(UUID_TEMPERATURE_UNIT)
    mug_connection.client.read_gatt_char.reset_mock()
    mug_connection.client.read_gatt_char.return_value = b'\x00'
...
async def test_set_mug_temperature_unit(mug_connection):
    ...
    mug_connection.client.write_gatt_char.assert_called_once_with(UUID_TEMPERATURE_UNIT, bytearray(b'\x00'))
...
async def test_get_mug_battery_voltage(mug_connection):
    ...
    mug_connection.client.read_gatt_char.assert_called_once_with(UUID_CONTROL_REGISTER_DATA)
...
def test_mug_notify_callback(mug_connection):
    mug_connection._notify_callback(None, 1, b'\x02')
    mug_connection._notify_callback(None, 1, b'\x02')
    assert mug_connection._latest_event_id == 2
    mug_connection._notify_callback(None, 1, b'\x04')
    assert mug_connection._latest_event_id == 4
    mug_connection._notify_callback(None, 1, b'\x05')
    assert mug_connection._latest_event_id == 5
    mug_connection._notify_callback(None, 1, b'\x06')
    assert mug_connection._latest_event_id == 6
    mug_connection._notify_callback(None, 1, b'\x07')
    assert mug_connection._latest_event_id == 7
    mug_connection._notify_callback(None, 1, b'\x08')
    assert mug_connection._latest_event_id == 8
    callback = Mock()
    mug_connection._notify_callback(callback, 1, b'\x09')
